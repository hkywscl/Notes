...
在 `config` 目录下新建一个 `paginate.php` 配置文件，代码如下：
...
在 `extend` 目录下创建一个 `page` 文件夹，然后在 `page` 文件夹里新建一个 `Page.php` 文件，代码如下：
